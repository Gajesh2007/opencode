...
You can connect with a [ChatGPT Plus or Pro](https://chatgpt.com/pricing) subscription or an OpenAI API key. These are separate providers so you can keep both connected and switch between them.

1. For a ChatGPT subscription, run the `/connect` command and select **Codex subscription**.

   ```txt
   /connect
   ```

2. Select the browser login option and authenticate with ChatGPT.
...
If you already have an API key, run `/connect`, select **OpenAI API**, and paste the key in your terminal. Connecting it does not replace your Codex subscription credentials.
